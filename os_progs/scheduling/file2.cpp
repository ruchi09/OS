...
// accordingly. This prog is same as file.cpp but with different commands (in main function)
...
int main()
{
    ...
    system("ls  > a.txt");
    FILE *fp,*fp1;
    char a[10],b[10],c[10],d[10],e[10],f[10],filename[20], line[50],lines[20],command[30];
    ...
    fp1 = popen(command,"r");
    fscanf(fp1,"%s %s %s %s %s %s %s %d:%d %s",a,b,c,d,line,e,f,&hr,&min,lines);
